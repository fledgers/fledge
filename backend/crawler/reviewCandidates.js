...
export function buildCandidateSummary(candidate) {
  ...
  return {
    ...
    visibility: candidate.visibility || opportunity.visibility || "public",
    owner: candidate.owner_user_id || opportunity.owner_user_id || "Not assigned",
    ...
    deadline:
      opportunity.deadline_source_text ||
      opportunity.deadline ||
      (opportunity.listing_expires_at
        ? `Rolling until ${opportunity.listing_expires_at}`
        : "Not stated"),
...
async function listPendingCandidates() {
  ...
  const candidates = await selectRows("opportunity_candidates", {
    select:
      "id,candidate_score,confidence_score,review_reasons,auto_publish_eligible,auto_publish_reasons,visibility,owner_user_id,change_count,source_url,application_url,raw_subject,extracted_opportunity,created_at",
    status: "eq.pending",
